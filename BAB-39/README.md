...
> | **FSM** | *Finite State Machine* — mesin kondisi berhingga: logika program yang bergerak antar state terdefinisi |
...
> | **AVR** | Keluarga mikrokontroler Atmel/Microchip yang digunakan di Arduino Uno/Nano |
> | **SRAM** | *Static Random-Access Memory* — RAM internal chip yang cepat tetapi hilang saat mati listrik |
...
> 📌 **API** (*Application Programming Interface*) adalah sekumpulan fungsi dan aturan yang disediakan oleh sebuah library agar dapat digunakan oleh programmer. Di sini, "API Preferences.h" berarti daftar lengkap fungsi yang bisa kamu panggil untuk berinteraksi dengan NVS.
 ...
 *   [CLI = Command Line Interface: antarmuka teks tempat pengguna
 *    mengetik perintah seperti "set bright 80" atau "reset"]
 *
...
> 📌 **FSM** (*Finite State Machine* — Mesin Kondisi Berhingga) adalah pola desain program di mana sistem hanya bisa berada di **satu state (kondisi) pada satu waktu**, dan berpindah antar state berdasarkan kejadian tertentu. Contoh: lampu lalu lintas berpindah MERAH → HIJAU → KUNING → MERAH secara berurutan. Di BAB ini, FSM digunakan untuk mengelola mode kerja perangkat: `IDLE`, `MONITOR`, dan `ALERT`.
...
✅ GUNAKAN NVS untuk:
  ...
  → Credential WiFi (SSID/*Service Set Identifier* — nama jaringan WiFi / Password)
...
│   (*tergantung konfigurasi OTA/*Over-The-Air* / partition scheme)   │
│                                                                       │
│ API PREFERENCES.H:                                                   │
